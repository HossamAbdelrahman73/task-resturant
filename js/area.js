...
async function getAreasSingle(area) {
  startSpinner();
  const api = await fetch(
...
function displayArea(areas) {
  ...
  for (let i = 0; i < areas.length; i++) {
    cartona += `
    <div class='area-icon' dat='${areas[i].strArea}'>
            <span
              ><i class="fa-solid fa-house-laptop font-[900] text-[64px]"></i
            ></span>
            <p class="font-[500] text-[28px]  ">${areas[i].strArea}</p>
     </div>
    `;
  }
  ...
  let dataArea = document.querySelectorAll(".area-icon");
  for (let i = 0; i < areas.length; i++) {
...
// document.getElementById('as').getAttribute('')
// function displayAreaSingle(data){
//   let cartona = ``;
//   for (let i = 0; i < areas.length; i++) {
//     cartona += `
//     <div class='area-icon'>
